...
RECON_MODEL_ROOT="$CODE_ROOT/eval_output/waymo_reconstruction_longtail_first49"
NVS_MODEL_ROOT="$CODE_ROOT/eval_output/waymo_nvs_longtail_first49"
EVAL_OUTPUT_ROOT="$CODE_ROOT/eval_output/longtail_region_metrics_first49"
...
case "$TASK" in
  train_recon)
    run_train "configs/waymo_reconstruction.yaml" "$RECON_MODEL_ROOT"
    ;;
  train_nvs)
    run_train "configs/waymo_nvs.yaml" "$NVS_MODEL_ROOT"
    ;;
  eval_recon)
    run_eval "configs/waymo_reconstruction.yaml" "$RECON_MODEL_ROOT" "reconstruction"
    ;;
  eval_nvs)
    run_eval "configs/waymo_nvs.yaml" "$NVS_MODEL_ROOT" "nvs"
    ;;
  all)
    run_train "configs/waymo_reconstruction.yaml" "$RECON_MODEL_ROOT"
    run_train "configs/waymo_nvs.yaml" "$NVS_MODEL_ROOT"
