...
RECON_CONFIG="configs/waymo_reconstruction_longtail_tuned_v2_first15.yaml"
NVS_CONFIG="configs/waymo_nvs_longtail_tuned_v2_first15.yaml"

RECON_MODEL_ROOT="$CODE_ROOT/eval_output/waymo_reconstruction_longtail_tuned_v2_first15"
NVS_MODEL_ROOT="$CODE_ROOT/eval_output/waymo_nvs_longtail_tuned_v2_first15"
EVAL_OUTPUT_ROOT="$CODE_ROOT/eval_output/longtail_tuned_v2_first15_metrics"
...
case "$TASK" in
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: bash $0 [GPU_ID] [recon_full|train_recon|train_nvs|eval_recon|eval_nvs|all] [RUN_TAG]" >&2
    exit 1
    ;;
esac
